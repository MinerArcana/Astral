...
public class InnerRealmChunkClaim implements IInnerRealmChunkClaim {
    ...
    @Override
    public boolean isChunkClaimedByPlayer(ServerPlayer player, ChunkPos chunk) {
    ...
    public boolean claimChunk(ServerPlayer player, ChunkPos chunk) {
        ...
        for (int i = 0; i < 4; i++) {
            LevelChunk adjacentChunk = InnerRealmUtils.getAdjacentChunk(chunk.getWorldPosition(), i, player.getCommandSenderWorld());
    ...
    public CompoundTag serializeNBT() {
        ...
        for (Map.Entry<UUID, List<ChunkPos>> entry : claimedChunksMap.entrySet()) {
            ...
            chunkListNBT.addAll(chunkPosList.stream().map(chunkPos -> NbtUtils.writeBlockPos(chunkPos.getWorldPosition())).collect(Collectors.toList()));
