...
public class BodyTracker implements IBodyTracker {
    ...
    private final Map<UUID, BodyInfo> bodyTrackerMap = new HashMap<>();
    ...
    @Override
    public Map<UUID, BodyInfo> getBodyTrackerMap() {
    ...
    public CompoundNBT serializeNBT() {
        ...
        bodyTrackerMap.forEach((uuid, bodyInfo) -> nbt.put(uuid.toString(), bodyInfo.serializeNBT()));
        return nbt;
    }
    ...
    public Optional<PhysicalBodyEntity> getBody(PlayerEntity playerEntity, ServerWorld world) {
        BodyInfo bodyInfo = bodyTrackerMap.get(playerEntity.getUniqueID());
        final ServerWorld bodyWorld = world.getServer().getWorld(bodyInfo.getDimensionType());
        if (!bodyTrackerMap.get(playerEntity.getUniqueID()).getBodyId().isPresent()){
            return Optional.empty();
        }
        else {
            return Optional.ofNullable((PhysicalBodyEntity) bodyWorld.getEntityByUuid(bodyTrackerMap.get(playerEntity.getUniqueID()).getBodyId().get()));
        }
    }
    ...
    @Override
    public void setBodyInfo(BodyInfo bodyInfo, PlayerEntity playerEntity) {
        bodyTrackerMap.put(playerEntity.getUniqueID(), bodyInfo);
    }
    ...
    public void mergeBodies(PlayerEntity playerEntity, ServerWorld world) {
        if (playerEntity instanceof ServerPlayerEntity) {
            ...
            if (bodyInfo != null) {
                ...
                if (getBody(playerEntity, world).isPresent()){
                    final PhysicalBodyEntity physicalBodyEntity = getBody(playerEntity, world).get();
    ...
    public void updatePlayer(ServerPlayerEntity playerEntity, ServerWorld world) {
        final BodyInfo bodyInfo = bodyTrackerMap.get(playerEntity.getUniqueID());
        if (bodyInfo != null) {
            if (!bodyInfo.isAlive()) {
    ...
    @Override
    public void removeBody(UUID playerID){
        final BodyInfo bodyInfo = bodyTrackerMap.get(playerID);
        if (bodyInfo != null){
            bodyInfo.setBodyId(null);
        }
    }
}
