...
public class EtherealTree extends Tree {
    ...
    public static final Supplier<TreeFeatureConfig> ETHEREAL_TREE_CONFIG = () -> {
        final TreeFeatureConfig treeBuilder = new TreeFeatureConfig.Builder(new SimpleBlockStateProvider(AstralBlocks.ETHEREAL_LOG.get().getDefaultState()), new SimpleBlockStateProvider(AstralBlocks.ETHEREAL_LEAVES.get().getDefaultState()), new BlobFoliagePlacer(2, 0)).baseHeight(5).heightRandA(2).foliageHeight(3).ignoreVines().setSapling(AstralBlocks.ETHEREAL_SAPLING.get()).build();
        treeBuilder.forcePlacement();
        return treeBuilder;
    };
