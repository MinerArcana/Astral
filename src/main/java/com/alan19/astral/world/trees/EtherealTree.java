...
public class EtherealTree extends Tree {
    ...
    public static final Supplier<BaseTreeFeatureConfig> ETHEREAL_TREE_CONFIG = () -> {
        final BaseTreeFeatureConfig config = Feature.TREE.withConfiguration(new BaseTreeFeatureConfig.Builder(new SimpleBlockStateProvider(AstralBlocks.ETHEREAL_LOG.get().getDefaultState()), new SimpleBlockStateProvider(AstralBlocks.ETHEREAL_LEAVES.get().getDefaultState()), new BlobFoliagePlacer(FeatureSpread.create(2), FeatureSpread.create(0), 3), new StraightTrunkPlacer(5, 2, 0), new TwoLayerFeature(2, 0, 1)).setIgnoreVines().build()).getConfig();
