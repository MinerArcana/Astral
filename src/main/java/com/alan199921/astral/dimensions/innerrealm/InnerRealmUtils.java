...
public class InnerRealmUtils {
    ...
    public void generateInnerRealmChunk(IWorld iWorld, IChunk iChunk) {
        ...
        for (x = 0; x < 16; x++) {
            for (z = 0; z < 16; z++) {
                iChunk.setBlockState(new BlockPos(x, iWorld.getSeaLevel(), z), AstralBlocks.egoMembrane.getDefaultState(), false);
                iChunk.setBlockState(new BlockPos(x, iWorld.getSeaLevel() + 15, z), AstralBlocks.egoMembrane.getDefaultState(), false);
            }
        }
